...
function GcodeFilesViewModel(printerStateViewModel, loginStateViewModel) {
    ...
    self.getPopoverContent = function(data) {
        ...
        if (data["gcodeAnalysis"]) {
            ...
            output += "<strong>Estimated Spray Time:</strong> " + data["gcodeAnalysis"]["estimatedPrintTime"];
            output += "</p>";
        }
        if (data["prints"] && data["prints"]["last"]) {
            output += "<p>";
            output += "<strong>Last Spray:</strong> <span class=\"" + (data["prints"]["last"]["success"] ? "text-success" : "text-error") + "\">" + data["prints"]["last"]["date"] + "</span>";
            output += "</p>";
        }
        return output;
    }
