...
def printerSpray():
	...
	if "cycles" in request.values.keys():
		spray_cycles = int(request.values["cycles"])
		# printer.command(";Cycles: " + spray_cycles)	

	if "delay" in request.values.keys():
		spray_delay = float(request.values["delay"])
		# printer.command(";Delay: " + spray_delay)
		
	if "solution" in request.values.keys():
	...
	file.write(sc_valve_spray + sc_speed.format(0.1) + sc_syringe_position.format(0))
	...
	for n in range(spray_cycles):
		...
		file.write(sc_speed.format(0.1) + sc_syringe_position.format(spray_syringe_travel))
